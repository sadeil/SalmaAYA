    ...
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-[1.4rem] border border-white/80 bg-white/85 px-5 py-3 shadow-card backdrop-blur-xl">
        ...
        <div className="hidden items-center gap-8 text-sm font-bold text-slate-600 md:flex">
          <a className="transition hover:text-teal-600" href="/#how">{t("How it works")}</a><a className="transition hover:text-teal-600" href="/#features">{t("Features")}</a><a className="transition hover:text-teal-600" href="/#safety">{t("Safety")}</a>
        </div>
        <div className="hidden items-center gap-2 md:flex"><LanguageToggle /><Link to="/login" className="btn-soft py-2.5">{t("Log in")}</Link><Link to="/register" className="btn-primary py-2.5">{t("Start your quest")}</Link></div>
      ...
      <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto mt-2 max-w-7xl rounded-3xl border border-white bg-white/95 p-4 shadow-soft backdrop-blur-xl md:hidden"><LanguageToggle /><Link to="/login" className="btn-soft mb-2 mt-3 w-full">{t("Log in")}</Link><Link to="/register" className="btn-primary w-full">{t("Start your quest")}</Link></motion.div>}</AnimatePresence>
    </header>
  );
}
  ...
  return (
    <div className="min-h-screen bg-mist">
      <aside className={`rq-sidebar fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-ink p-5 text-white transition-transform lg:translate-x-0 ${open ? "translate-x-0" : isArabic ? "translate-x-full" : "-translate-x-full"}`}>
        ...
        <div className="my-7 rounded-3xl border border-white/5 bg-white/8 p-4 shadow-inner">
          <div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 text-white shadow-lg shadow-black/10"><RoleIcon size={20} /></span><div><p className="text-sm font-extrabold">{t(roleData[0])}</p><p className="text-xs text-white/50">{t(roleData[1])}</p></div></div>
        </div>
        <nav className="space-y-1">
          {links.map(([to, label, Icon], index) => <NavLink key={`${to}-${index}`} end={index === 0} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `group flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition ${isActive ? "bg-gradient-to-r from-teal-500 to-teal-600 text-white shadow-lg shadow-black/10" : "text-white/60 hover:bg-white/5 hover:text-white"}`}><Icon size={18} className="transition-transform group-hover:scale-110" />{t(label)}</NavLink>)}
        </nav>
        <div className="mt-auto rounded-3xl border border-white/5 bg-gradient-to-br from-teal-500/30 to-violet-400/20 p-4"><Sparkles className="mb-3 text-teal-100" /><p className="text-sm font-extrabold">{t("Keep showing up")}</p><p className="mt-1 text-xs leading-5 text-white/55">{t("Small movements build lasting recovery.")}</p></div>
        <Link to="/" className="mt-3 flex items-center gap-3 px-4 py-3 text-sm font-bold text-white/50 transition hover:text-white"><LogOut size={18} /> {t("Sign out")}</Link>
      </aside>
      <main className="rq-main lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/80 bg-mist/80 px-4 shadow-[0_8px_30px_rgba(23,58,58,.03)] backdrop-blur-xl md:px-8">
          ...
          <div className="hidden lg:block"><p className="text-xs font-bold uppercase tracking-[.18em] text-slate-400">{t(`${role} space`)}</p><p className="text-sm font-extrabold capitalize">{t(links.find(([to]) => to === location.pathname)?.[1] || "RemedyQuest")}</p></div>
...
export function Footer() {
  const { t } = useLanguage();
  return <footer className="relative overflow-hidden bg-ink px-6 py-10 text-white"><div className="absolute -bottom-24 end-10 h-56 w-56 rounded-full bg-teal-500/10 blur-3xl" /><div className="relative mx-auto flex max-w-7xl flex-col justify-between gap-6 md:flex-row md:items-center"><Brand light /><p className="max-w-md text-sm leading-6 text-white/50">{t("General therapeutic exercise guidance. RemedyQuest does not replace professional medical advice.")}</p><p className="text-xs text-white/40">© 2026 RemedyQuest</p></div></footer>;
}
