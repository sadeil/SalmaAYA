  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-[1.4rem] border border-white/80 bg-white/85 px-5 py-3 shadow-card backdrop-blur-xl">
        ...
        <div className="hidden items-center gap-2 md:flex"><Link to="/login" className="btn-soft py-2.5">Log in</Link><Link to="/register" className="btn-primary py-2.5">Start your quest</Link></div>
      ...
      <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto mt-2 max-w-7xl rounded-3xl bg-white p-4 shadow-soft md:hidden"><Link to="/login" className="btn-soft mb-2 w-full">Log in</Link><Link to="/register" className="btn-primary w-full">Start your quest</Link></motion.div>}</AnimatePresence>
    </header>
  );
}
    ...
    <div className="min-h-screen bg-mist">
      ...
      <main className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200/60 bg-mist/85 px-4 backdrop-blur-xl md:px-8">
          ...
          <div className="ml-auto flex items-center gap-2"><Link to="/" className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-slate-500 shadow-card"><Home size={18} /></Link><button className="flex items-center gap-3 rounded-2xl bg-white px-3 py-2 shadow-card"><span className="grid h-8 w-8 place-items-center rounded-xl bg-teal-100 text-xs font-extrabold text-teal-700">{roleData[0].split(" ").map(x => x[0]).join("").slice(0,2)}</span><ChevronDown size={15} /></button></div>
        </header>
